...
export function ThemeToggle({
    ...
    const container = cn('inline-flex cursor-(--cursor-interactive) items-center rounded-full border p-1', className)
        ...
        return (
            ...
            >
                {full.map(([key, Icon]) => {
                    ...
                    return (
                        <Icon key={key} className={cn(itemVariants({ active: value === key }))} fill="currentColor" />
                    )
                })}
            </button>
        )
