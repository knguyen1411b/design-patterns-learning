...
export function PageLastUpdate({ date: value, ...props }: Omit<ComponentProps<'p'>, 'children'> & { date: Date }) {
    ...
    return (
        <p {...props} className={cn('text-fd-muted-foreground text-sm', props.className)} suppressHydrationWarning>
            {text.lastUpdate} {value.toLocaleDateString()}
        </p>
    )
}
...
export function PageBreadcrumb({ includePage, includeRoot, includeSeparator, ...props }: BreadcrumbProps) {
    const path = useTreePath()
    const { root } = useTreeContext()
    const items = useMemo(() => {
        return getBreadcrumbItemsFromPath(root as PageTree.Root, path as PageTree.Node[], {
            includePage,
            includeRoot,
            includeSeparator
        })
    ...
    return (
        <div {...props} className={cn('text-fd-muted-foreground flex items-center gap-1.5 text-sm', props.className)}>
            {items.map((item, i) => {
