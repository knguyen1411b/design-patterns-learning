...
const itemVariants = tv({
    ...
    variants: {
        ...
        variant: {
            button: 'hover:bg-fd-accent/50 hover:text-fd-accent-foreground/80 transition-colors hover:transition-none',
...
export const { SidebarCollapseTrigger, SidebarFolder, SidebarProvider: Sidebar, SidebarTrigger, SidebarViewport } = Base

export function SidebarContent({ children, className, ref: refProp, ...props }: ComponentProps<'aside'>) {
            ...
            {({ collapsed, hovered, ref: asideRef, ...rest }) => (
                ...
                >
                    ...
                    <aside
                        ...
                        id="nd-sidebar"
                        className={cn(
                            'absolute inset-y-0 inset-s-0 flex w-full flex-col items-end text-sm duration-250 *:w-(--fd-sidebar-width)',
    ...
    return (
        <>
            ...
            <Base.SidebarDrawerContent
                className={cn(
                    'bg-fd-background data-[state=open]:animate-fd-sidebar-in data-[state=closed]:animate-fd-sidebar-out fixed inset-y-0 inset-e-0 z-40 flex w-[85%] max-w-95 flex-col text-[0.9375rem] shadow-lg',
                    className
                )}
                {...props}
            >
...
export function SidebarItem({ children, className, style, ...props }: ComponentProps<typeof Base.SidebarItem>) {
    const depth = Base.useFolderDepth()
